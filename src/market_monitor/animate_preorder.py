"""Animate Online Shop pre-order / lottery crawler.

Target page:
  - 予約商品一覧: https://www.animate-onlineshop.jp/sp/fair/reserve/

Animate's pre-order listing page uses a product grid with article.itemCardBox
elements.
"""
...
_ENTRY_URLS = (
    "https://www.animate-onlineshop.jp/sp/fair/reserve/",
    "https://www.animate-onlineshop.jp/spr/commodities/index/?searchkey=%E3%83%88%E3%83%AC%E3%82%AB&so=newer",
)
...
@dataclass
class AnimatePreorderCrawler:
    """Crawls Animate Online Shop's pre-order / lottery TCG listings."""
    ...
    def fetch_listings(self, *, timeout_seconds: int = 30) -> list[OfficialStoreListing]:
        ...
        for r in results:
            if r.item_key not in seen:
                seen.add(r.item_key)
                deduped.append(r)
        logger.info("AnimatePreorderCrawler: fetched listings=%d", len(deduped))
        return deduped
    ...
    def _fetch_page(self, url: str, *, timeout_seconds: int) -> list[OfficialStoreListing]:
        ...
        return parse_animate_page(soup, base_url=url)


def parse_animate_page(
    soup: "BeautifulSoup", *, base_url: str
) -> list[OfficialStoreListing]:
    """Parse Animate Online Shop product listing pages.

    Animate uses article.itemCardBox or li.item elements. We try selectors in
    priority order and stop at the first that yields results, to avoid
    double-counting nested elements."""
    _SELECTORS = (
        "article.itemCardBox",
        "div.item-card",
        "li.product",
    ...
    for selector in _SELECTORS:
        ...
        break  # stop after first selector that matches
...
def _parse_card(card: "BeautifulSoup", *, base_url: str) -> OfficialStoreListing | None:
    ...
    title_tag = card.select_one(".itemName, .item-name, .title, h3, h4")
    ...
    if status == "unknown":
        # Animate's reserve listing page implies preorder_open by default
