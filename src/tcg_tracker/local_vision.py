...
class OllamaLocalVisionClient:
    ...
    def _build_sealed_box_title_prompt(self, *, game_hint: str | None, title_hint: str | None) -> str:
        ...
        return (
            ...
            "PRIMARY TARGET: the biggest printed set-name text on the box front — the unique identifier that makes this specific product different from every other sealed box in the same product line. That printed set-name text is what to return as title.\n"
            ...
            "Return EXACTLY what is printed on this specific box front. Shapes you may encounter (described generically — do NOT echo these descriptions as the title):\n"
            "  - A box-front set name (most common): the unique product identifier printed in the largest typeface. Boxes that print Roman words (MEGA / SUPER / GIGA / etc.) keep those words in Roman with a space before the rest of the set name. Katakana set names stay as katakana. Roman set names stay as Roman.\n"
            "  - A short numeric set token when that number is the only big title text on the box front.\n"
            "  - A combined phrase joining a Japanese product-line word with a set name when BOTH are printed together as one continuous block on the box front.\n"
            "ANTI-PATTERNS — return title=null instead of doing any of these:\n"
            "  - Do NOT default to a bare generic Japanese product-line word (the word that appears on EVERY box in the same product line — e.g. words ending in 拡張パック, ハイクラスパック, スタートデッキ, スターターセット, トレーナーボックス, デッキビルドBOX) when you cannot actually read the specific set name on this box. That word alone is not the title — return null and let the caller ask the user.\n"
            "  - Do NOT return a slash-separated or comma-separated list of multiple product-line phrases as the title. That syntax is example notation in instructions, not printed text on a real box. If you would output something like \"<word> / <word>\" or a list of alternatives, return title=null.\n"
            "  - Do NOT expand a short numeric set token into a longer guessed product line.\n"
            "  - Do NOT invent set names from prior knowledge (e.g. never output VSTAR Universe, VSTAR Unlimited, LEGENDARY SPARK, or any English product name unless those exact words are clearly printed on this box).\n"
            "  - Do NOT transliterate Roman letters into katakana or vice versa. Preserve the original Roman vs katakana form the box uses.\n"
            "If only a partial set-name fragment is clearly visible on the box front, return that exact fragment as title instead of inventing missing words.\n"
